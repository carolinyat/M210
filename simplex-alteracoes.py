...
class SimplexApp:
    ...
    def create_widgets(self):
        ...
        self.button_post = ttk.Button(self.root, text="Pós-Otimização", command=self.post_optimization, width=20, style='TButton')
        self.button_post.grid(row=14, column=1, padx=5, pady=5, sticky=tk.W + tk.E)
        ...
        if result.success:
            ...
            if A and b:
                ...
                if sombra_result.success:
                    self.preco_sombra = sombra_result.x  
                    preco_sombra = [f"{val:.2f}" for val in self.preco_sombra]
                    self.output.insert(
    ...
    def post_optimization(self):
        if not hasattr(self, 'result') or not self.result.success:
            messagebox.showwarning("Aviso", "Resolva o problema primeiro.")
            return

        try:
            delta_str = simpledialog.askstring("Pós-Otimização", f"Digite os Δs das {len(self.b_usadas)} restrições (separados por espaço):")
            deltas = list(map(float, delta_str.replace(',', '.').split()))

            if len(deltas) != len(self.b_usadas):
                raise ValueError

            viavel = True
            z_novo = self.result.fun

            for i, delta in enumerate(deltas):
                slack = self.result.slack[i] if hasattr(self.result, 'slack') else 0
                sombra = self.preco_sombra[i] if hasattr(self, 'preco_sombra') else 0
            ...
            if viavel:
                self.output.insert(tk.END, f"\nAlterações viáveis. Novo lucro ótimo: R$ {z_novo:.2f}\n")
            else:
                self.output.insert(tk.END, "\nAlterações não são viáveis. Algumas violam a folga das restrições.\n")

        except Exception:
            messagebox.showerror("Erro", "Entrada inválida para os Δs.")
